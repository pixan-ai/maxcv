...
function generateAndDownloadPdf(text: string) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>maxcv</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;font-size:11pt;line-height:1.6;color:#1a1a1a;padding:40px 50px;max-width:800px;margin:0 auto}pre{white-space:pre-wrap;word-wrap:break-word;font-family:inherit;font-size:inherit;line-height:inherit}@media print{body{padding:0}@page{margin:2cm}}</style></head><body><pre>${text.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}</pre><script>window.onload=function(){setTimeout(function(){window.print();window.close()},500)};<\/script></body></html>`);
  w.document.close();
}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-ink-000">
      ...
      <main className="flex-1 w-full max-w-2xl mx-auto px-5 py-8">
        ...
        {!result && <div className="mb-6"><CVInput cvText={cvText} setCvText={setCvText} targetRole={targetRole} setTargetRole={setTargetRole} onSubmit={handleSubmit} loading={loading} error={error} setError={setError} isEs={lang === "es"} submitLabel={t.button} loadingLabel={t.loading} /></div>}
        ...
        {result && (
          <div ref={resultRef} className="space-y-6">
            <div className="bg-white border border-ink-100 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-ink-100 bg-ink-050">
                ...
                <div className="flex gap-3">
                  <button onClick={async () => { if (!result) return; await navigator.clipboard.writeText(result.improved); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                  ...
                  <button onClick={() => result && generateAndDownloadPdf(result.improved)}
                    className="text-xs text-ink-400 hover:text-ink-700 font-medium transition cursor-pointer">{t.downloadPdf}</button>
                </div>
              </div>
              <pre className="whitespace-pre-wrap text-sm leading-relaxed text-ink-900 p-5 sm:p-6">{result.improved}</pre>
            </div>
            ...
            <button onClick={() => { setResult(null); setCvText(""); setTargetRole(""); }}
              className="w-full bg-ink-900 text-white font-medium py-3.5 px-6 rounded-xl hover:bg-ink-700 transition-colors cursor-pointer text-sm">{t.tryAgain}</button>
