...
const T = {
  en: { tabText: "Paste text", tabFile: "Upload PDF", tabSheets: "Google Sheets", labelCv: "Your resume", placeholderCv: "Paste your resume text here...", chars: "chars", labelRole: "Target role", optional: "optional", placeholderRole: "e.g. Product Manager, Software Engineer...", uploadTitle: "Drop your PDF here", uploadMeta: "PDF only \u00b7 max 5MB \u00b7 Word? Save as PDF first", parsing: "Reading your file...", parseError: "Could not read the file. Try pasting your resume instead.", uploadSuccess: "File loaded", changeFile: "Change", clearText: "Clear", sheetsLabel: "Google Sheets link", sheetsPlaceholder: "https://docs.google.com/spreadsheets/d/...", sheetsHint: "The sheet must be shared as \"Anyone with the link\"", sheetsButton: "Import", sheetsLoading: "Importing...", sheetsError: "Could not access Google Sheet.", fileTooLarge: "File is too large. Maximum 5MB.", fileWrongType: "Only PDF files accepted.", privacy: "Your CV is processed in real-time and never stored." },
...
export default function CVInput({ cvText, setCvText, targetRole, setTargetRole, onSubmit, loading, error, setError, isEs, submitLabel, loadingLabel }: CVInputProps) {
  ...
  const handleSheetsImport = useCallback(async () => {
    ...
    try {
      const form = new FormData(); form.append("sheetsUrl", sheetsUrl);
      const res = await fetch("/api/parse", { method: "POST", body: form });
      const data = await res.json();
  ...
  const resetFile = () => { setFileName(""); setFileLoaded(false); setCvText(""); if (fileRef.current) fileRef.current.value = ""; };

  const tabs: { mode: InputMode; label: string }[] = [
    { mode: "file", label: t.tabFile }, { mode: "text", label: t.tabText }, { mode: "sheets", label: t.tabSheets },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex border-b border-ink-100">
        {tabs.map(({ mode, label }) => (
          <button key={mode} type="button" onClick={() => setInputMode(mode)}
            className={`flex-1 px-3 py-2.5 text-sm transition cursor-pointer -mb-px ${inputMode === mode ? "text-accent border-b-2 border-accent font-medium" : "text-ink-400 hover:text-ink-700"}`}>
            {label}
          </button>
        ))}
      </div>
          ...
          {fileLoaded && fileName ? (
            <div className="flex items-center gap-3 bg-accent-ghost border border-accent/15 rounded-lg px-5 py-4">
              <div className="w-8 h-8 rounded-md bg-accent/10 flex items-center justify-center">
                <svg className="w-4 h-4 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-ink-900">{t.uploadSuccess}</p>
                <p className="text-xs text-ink-500 truncate">{fileName}</p>
              </div>
              <button type="button" onClick={resetFile} className="text-xs text-ink-500 hover:text-ink-900 font-medium transition cursor-pointer">{t.changeFile}</button>
            </div>
          ) : (
            <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}
              onClick={() => !uploading && fileRef.current?.click()}
              className={`border rounded-xl py-6 px-6 text-center cursor-pointer transition-all ${dragOver ? "border-accent bg-accent-ghost scale-[1.01]" : "border-ink-200 bg-accent-ghost/50 hover:border-accent/30 hover:bg-accent-ghost"}`}>
              ...
              {uploading ? (
                ...
                <>
                  <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center mx-auto mb-3">
                    <svg className="w-5 h-5 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" /></svg>
                  </div>
      ...
      {inputMode === "text" && (
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {cvText.length > 0 && <button type="button" onClick={() => { setCvText(""); setFileName(""); setFileLoaded(false); }} className="text-xs text-ink-400 hover:text-ink-700 transition cursor-pointer">{t.clearText}</button>}
          </div>
          <textarea id="cv-input" rows={12} value={cvText} onChange={(e) => setCvText(e.target.value)} placeholder={t.placeholderCv} required
          ...
          {cvText.length > 0 && <p className="text-xs text-ink-400 mt-1.5">{cvText.length.toLocaleString()} {t.chars}{fileName ? ` \u2014 ${fileName}` : ""}</p>}
        </div>
      )}
      ...
      {inputMode === "sheets" && (
        <div className="space-y-3">
          <div>
            ...
            <input id="sheets-url" type="url" value={sheetsUrl} onChange={(e) => setSheetsUrl(e.target.value)} placeholder={t.sheetsPlaceholder}
          ...
          <button type="button" onClick={handleSheetsImport} disabled={sheetsLoading || !sheetsUrl.trim()}
            className={`w-full font-medium py-3 px-6 rounded-xl transition-colors cursor-pointer ${sheetsLoading || !sheetsUrl.trim() ? "bg-ink-100 text-ink-300 cursor-not-allowed" : "bg-accent text-white hover:bg-accent-dim"}`}>
      ...
      <div>
        ...
        <input id="target-role" type="text" value={targetRole} onChange={(e) => setTargetRole(e.target.value)} placeholder={t.placeholderRole}
          className="w-full rounded-xl border border-ink-200 bg-white px-4 py-3 text-sm text-ink-900 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition placeholder:text-ink-300" />
      </div>
...
function Spinner({ label }: { label: string }) {
  return <span className="flex items-center justify-center gap-2"><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{label}</span>;
}
